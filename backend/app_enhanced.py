...
International Users: {(df['intl_total_calls'] > 0).sum():,}

Time Distribution:
- Morning Calls: {df['voice_morning_calls'].sum():,.0f}
- Evening Calls: {df['voice_evening_calls'].sum():,.0f}
- Night Calls: {df['voice_night_calls'].sum():,.0f}

SMS:
- Total Messages: {df['sms_total_messages'].sum():,.0f}
- Average per User: {df['sms_total_messages'].mean():.1f}

Data Usage:
- Total Download (GB): {df['data_downlink_mb'].sum() / 1024:.1f}
- Total Upload (GB): {df['data_uplink_mb'].sum() / 1024:.1f}
- Average per User (MB): {df['data_total_mb'].mean():.1f}
